...
def details(link):
    source_code = requests.get("https://www25.gogoanimes.tv/" + link)
...
def links(title, link):
    for i in range(len(title)):
        print("%d. %s : https://www25.gogoanimes.tv/%s\n" % (i + 1, title[i], link[i]))


def entry():
    anime_name = input("[+] Enter the name of the Anime : ")
    search_url = ("https://https://www25.gogoanimes.tv//search.html?keyword=" + anime_name)
